/*mic window manager is designed like any other X client as well. It is
...
void drawbar(Monitor *m) {
  ...
  x = drw_text(drw, x, 0, w, bh, lrpad / 8, m->ltsymbol, 0);
  ...
  if (m == selmon || 1) { /* status is only drawn on selected monitor */
    ...
    if (splitstatus) {
      ...
      drw_text(drw, msx, 0, TEXTW(mstext) - lrpad, bh, 0, mstext, 0);
		...
		x = 0;
for (text = s = rstext ? rstext : stext; *s; s++) {		
      if ((unsigned char)(*s) < ' ') {
				ch = *s;
				*s = '\0';
				tw = TEXTW(text) - lrpad;
...
char *text, *s, ch;
char *stextcopy = strdup(stext);
char *rpart = stextcopy;
if (splitstatus)
    strsep(&rpart, splitdelim);
statusw = 0;
for (text = s = rpart ? rpart : stextcopy; *s; s++) {
    if ((unsigned char)(*s) < ' ') {
        ch = *s;
        *s = '\0';
